...
public class SGDFOP<Model: EuclideanDifferentiable>: Optimizer
    ...
    public init(
        ...
        for kp in matrix.recursivelyAllWritableKeyPaths(to: Tensor<Float>.self) {
            ...
            if t.rank == 4 {
                matrix[keyPath: kp] = _Raw.diag(diagonal: Tensor<Float>(ones: [t.shape[0] * t.shape[1]]))
                //print(matrix[keyPath: kp].shape, matrix[keyPath: kp])
                fopVelocity[keyPath: kp] = Tensor<Float>(zeros: matrix[keyPath: kp].shape)
            }
            else if t.rank == 2 {
                let std = sqrt(rsqrt(Tensor<Float>(Float(t.shape[0]))))
                //matrix[keyPath: kp] = Tensor<Float>(randomNormal: [t.shape[0], t.shape[0]], 
                //                                    standardDeviation: Tensor<Float>(1e-2))
                matrix[keyPath: kp] = _Raw.diag(diagonal: Tensor<Float>(ones: [t.shape[0]]))
        ...
        for kp in fopDirection.recursivelyAllWritableKeyPaths(to: Tensor<Float>.self) {
            ...
            if dir.rank > 1 {
                ...
                if dir.rank == 4 {
                    ...
                    let eye = _Raw.diag(diagonal: Tensor<Float>(ones: [pcm.shape[0]]))
                    fopDirection[keyPath: kp] = matmul(pcm + eye, dir)
